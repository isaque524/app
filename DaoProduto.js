...
export default class DaoProduto {
  ...
  async alterar(produto) {
    ...
    let resultado = new Promise(function(resolve, reject) {
      ...
      var keyValue = IDBKeyRange.only(produto.getcormercial());
      indice.openCursor(keyValue).onsuccess = event => {
        const cursor = event.target.result;
        if (cursor) {
          if (cursor.value.comercial == produto.getcormercial()) {
          ...
          reject(new ModelError("Produto com o nome " + produto.getcormercial() + " não encontrado!",""));
        }
      };
    });
    return await resultado;
